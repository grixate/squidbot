...
func (r *Runtime) registerChannels(cfg config.Config) {
	...
	for channelID, channelCfg := range cfg.Channels.Registry {
		...
		case "telegram":
			...
			adapter := &telegramAdapter{id: "telegram", channel: telegram.New(telegramCfg, func(ctx context.Context, msg agent.InboundMessage) error {
	...
	if !hasTelegram && cfg.Channels.Telegram.Enabled {
		adapter := &telegramAdapter{id: "telegram", channel: telegram.New(cfg.Channels.Telegram, func(ctx context.Context, msg agent.InboundMessage) error {
			_, err := r.Engine.Submit(ctx, msg)
			return err
		}, r.log)}
		if err := r.Channels.Register(adapter); err != nil {
